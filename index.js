...
function valid(req, res, next) {
    if (!req.validated) throw createError(500, "server has not valdiated request")
    if (req.errorMessage) throw createError(400, req.errorMessage);
    next();
}
...
app.post("/api/v1/courses", validator(schemaPost), (req, res) => {
    ...
    if (courses[id]) throw createError(400, `course with id ${id} already exists`);
    courses[id] = req.body;
    console.log(courses);
